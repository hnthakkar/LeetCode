package backtracking.WordLadderII126;
...
public class Solution {
    ...
    public static void main(String[] args) {
        ...
        // sol.ladderLength("hit", "cog", Arrays.asList(new String[]{"hot", "dot", "dog", "lot", "log", "cog"}));
        // sol.findLadders("a", "c", Arrays.asList(new String[]{"a", "b", "c"}));
    ...
    public List<List<String>> findLadders(String beginWord, String endWord, List<String> wordList) {
        ...
        Map<String, Set<String>> nb = new HashMap<>();
    ...
    private void getNeighbours(String forWord, List<String> wordList, Map<String, Set<String>> nb, Map<String, char[]> wordCharRef, int startAt, int wLen) {
        char[] forWordArray = wordCharRef.get(forWord);

        for (int i = startAt; i < wLen; i++) {
            String toWord = wordList.get(i);
            if (forWord.equals(toWord)) {
                continue;
            }
            ...
            if (validateNeighbour(forWordArray, toWordArray)) {
                if (!nb.containsKey(forWord)) {
                    nb.put(forWord, new HashSet<String>());
                }

                Set<String> fromNList = nb.get(forWord);
                fromNList.add(toWord);

                if (!nb.containsKey(toWord)) {
                    nb.put(toWord, new HashSet<String>());
                }

                Set<String> toNList = nb.get(toWord);
                toNList.add(forWord);
            }
        }
    }
