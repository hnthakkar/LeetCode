package matrix.PacificAtlanticWaterFlow417;
...
public class Solution {
    ...
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        for (int r = 0; r < rows; r++) {
            helper(r, 0, heights,pReachable, rows, cols);
            helper(r, cols - 1, heights, aReachable, rows, cols);
        }

        for (int c = 0; c < cols; c++) {
            helper(0, c, heights, pReachable, rows, cols);
            helper(rows - 1, c, heights, aReachable, rows, cols);
        }
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                if (pReachable[r][c] && aReachable[r][c]) {
    ...
    private void helper(int curR, int curC, int[][] heights, boolean[][] reachable, int rows, int cols) {
        if (reachable[curR][curC]) {
            return;
        }
        ...
        // up
        if (curR > 0 && heights[curR][curC] <= heights[curR - 1][curC]) {
            helper(curR - 1, curC, heights, reachable, rows, cols);
        }

        // down
        if (curR < (rows - 1) && heights[curR][curC] <= heights[curR + 1][curC]) {
            helper(curR + 1, curC, heights, reachable, rows, cols);
        }

        // left
        if (curC > 0 && heights[curR][curC] <= heights[curR][curC - 1]) {
            helper(curR, curC - 1, heights, reachable, rows, cols);
        }

        // right
        if (curC < (cols - 1) && heights[curR][curC] <= heights[curR][curC + 1]) {
            helper(curR, curC + 1, heights, reachable, rows, cols);
        }
    }
    /*
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        // 0 -> not checked yet
        // -1 -> can't visit both
        // 1 -> can visit Pacific Ocean
        // 2 -> can visit Atlantic Ocean
        // 3 (1 + 2) -> can visit both
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {
                //if (dp[r][c] == 0) {
                    helper(r, c, dp, heights, visited, rows, cols);
                //}
    ...
    private void helper(int curR, int curC, int[][] dp, int[][] heights, int[][] visited, int rows, int cols) {
        ...
        // up
        if (curR > 0 && heights[curR][curC] >= heights[curR - 1][curC]) {
            helper(curR - 1, curC, dp, heights, visited, rows, cols);
            updateDP(curR, curC, curR - 1, curC, dp);
        }
        ...
        // left
        if (curC > 0 && heights[curR][curC] >= heights[curR][curC - 1]) {
            helper(curR, curC - 1, dp, heights, visited, rows, cols);
            updateDP(curR, curC, curR, curC - 1, dp);
        }
    ...
    private void updateDP(int curX, int curY, int nextX, int nextY, int[][] dp) {
        if (dp[curX][curY] == 3) {
            return;
        }
